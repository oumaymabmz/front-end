...
const HouseList = ({ houses, reviews, onAddReview, bookedItems, onAddToCart }) => {
  const handleReviewSubmit = (itemTitle, reviewText, rating) => {
    onAddReview('houses', itemTitle, reviewText, rating);
  };

  const isBooked = (itemTitle) => {
    return bookedItems.some(item => item.type === 'houses' && item.itemTitle === itemTitle);
  };

  return (
    <div className="bg-white p-6 rounded shadow-md container mx-auto">
      <h2 className="text-2xl font-bold mb-4 text-center">House List</h2>
        ...
        {houses.length === 0 ? (
          ...
          houses.map((house, index) => (
            <div key={index} className="max-w-sm bg-white border border-gray-200 rounded-lg shadow-md overflow-hidden m-4 transform transition-transform hover:scale-105">
              ...
              <div className="p-4">
                <h5 className="text-xl font-semibold tracking-tight text-gray-900">{house.title}</h5>
                <p className="text-sm text-gray-600">{house.description}</p>
                <p className="text-sm text-gray-600">Location: {house.location}</p>
                <p className="text-sm text-gray-600">Price: ${house.price}</p>
                <div className="mt-4">
                  <button
                    onClick={() => onAddToCart('houses', house.title)}
                    className={`px-4 py-2 rounded w-full ${isBooked(house.title) ? 'bg-gray-500' : 'bg-green-500'} text-white`}
                    disabled={isBooked(house.title)}
                  >
                    {isBooked(house.title) ? 'Booked' : 'Book Now'}
                  </button>
                </div>
                <ReviewForm itemTitle={house.title} onSubmit={handleReviewSubmit} />
                <div className="mt-4">
                  <h4 className="text-lg font-semibold">Reviews</h4>
                  {reviews.filter(review => review.type === 'houses' && review.itemTitle === house.title).length === 0 ? (
                    <p>No reviews yet.</p>
                  ) : (
                    reviews.filter(review => review.type === 'houses' && review.itemTitle === house.title).map((review, index) => (
...
const HouseCarousel = ({ images }) => {
  ...
  return (
    <div className="relative">
      <img className="w-full h-48 object-cover transition-opacity duration-300 ease-in-out" src={images[currentIndex]} alt={`House ${currentIndex}`} />
      {images.length > 1 && (
        <>
